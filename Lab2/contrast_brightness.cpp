...
int main(int argc, char** argv) {
	...
	Mat color_img;
	Mat::zeros(color_img.size(), image.type());
	...
	for (int i = 0; i < h; i++) {
		for (int j = 0; j < w; j++) {
			...
			color_img.at<Vec3b>(j, i)[0] = saturate_cast<uchar>(blue);
			color_img.at<Vec3b>(j, i)[1] = saturate_cast<uchar>(green);
			color_img.at<Vec3b>(j, i)[2] = saturate_cast<uchar>(red);
		}
	}
